...
const Gpt = () => {
   ...
   useEffect(() => {
      socket.on('newMessage', async (data) => {
         ...
         data.content === undefined && dataRef.current[dataRef.current.length - 1] !== ''
            ? setIsWrite(false)
            : setIsWrite(true);
      });
   ...
   return (
      <div
         style={{ height: height }}
         className={`bg-slate-800 flex justify-center perspective overflow-hidden`}
      >
         <div
            className={`bg-neutral-200 relative w-full m-4 md:mx-24 lg:mx-44 xl:mx-60 flex flex-col justify-between rounded-3xl 
         ...
         >
            <div className='flex justify-between py-4 px-6 shadow-sm shadow-slate-300 rounded-t-3xl'>
               ...
               <div className='flex flex-col items-center'>
                  ...
                  <span className='text-black text-opacity-50 text-xs -mt-1'>
               ...
               {chat &&
                  chat.map((item, index) => {
                     return (
                        <div
                           key={index}
                           className={`flex items-center ${item.text === '' && 'hidden'} ${
                           ...
                           >
                              ...
                              <span
                                 className={`text-[8px] sm:text-[10px] ml-auto -mb-[3px] mt-1 ${
                                    item.user === 'user' ? 'text-white' : 'text-slate-800'
                                 } opacity-40`}
                              >
                                 {item.time}
                              </span>
